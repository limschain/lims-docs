...
点击{实验室信息管理}-->{委托管理}-->{委托列表}菜单，弹出[委托列表页面](#委托列表页面)，如下图所示：
...
<html><span id='orders/NewOrder'></span></html>
  ...
  1.  **创建委托单**  点击【创建委托单】按钮，[录入委托单基本信息](#orders/new)。
...
<html><span id='orders/PrintContract'></span></html>
  ...
  3.  **打印** 点击当前委托单右端【操作】-->【打印委托单】，[执行打印](#orders/review)。
...
实验室用户在Web端登录基于区块链的检验检测管理云平台（LIMSChain）后，手工创建委托单。操作步骤如下表所示：
| 步骤 | 描述| 备注 |是否必须 | 
| --- | --- | --- |--- |
...
**操作路径一：** 直接进入[创建委托单](#orders/new)：点击{实验室信息管理}-->{委托管理}-->{委托登记}菜单，弹出[委托单基本信息页面](#orders/new-page)，如下图所示：

**操作路径二：** 在委托列表中登记：点击{实验室信息管理}-->{委托管理}-->{委托列表}-->【创建】，弹出[委托单基本信息页面](#orders/new-page)，如下图所示：

<html><span id='orders/new-page'></span></html>

![委托登记-委托单基本信息页面](http://datmfiles.ebookchain.org/1JCo4Edge7L%E5%A7%94%E6%89%98%E7%AE%A1%E7%90%86-%E5%A7%94%E6%89%98%E5%88%97%E8%A1%A8-%E5%88%9B%E5%BB%BA%E5%A7%94%E6%89%98%E5%8D%95%E6%93%8D%E4%BD%9C%E9%A1%B5%E9%9D%A2.png "委托单基本信息页面")
...
**5.保存委托单信息**
以上三部分信息录入完成之后，点击【保存，添加样品信息】按钮，保存信息到云端，同时弹出。如果需要采样或者抽样，则[登记样品类型信息](#登记样品类型信息（需要抽样/采样）)；否则[登记送检样品信息](#登记样品信息（无需抽样/采样）)。

<html><span id='登记样品信息（无需抽样/采样）'></span></html>
...
- **手工登记** 在[保存并添加样品操作页面](#orders/new/SampleInfo/page)中点击【添加样品】，弹出[新增样品操作页面](#新增样品操作页面)，如下图所示：
...
样品类别信息是根据动态表单自动生成的。具体操作如下：在[样品类别信息录入页面](#orders/new/SampleTypeInfo/page)中，输入样品类别信息，添加样品类别信息，即可完成样品类别信息的添加。
...
**第一步** 点击{实验室信息管理}-->{委托管理}-->{委托列表}菜单，弹出[委托列表](#委托列表)。

**第二步** 在[委托列表](#委托列表)中点击当前委托单右端【操作】-->【提交审核】，[执行委托审核](#orders/review)。

- 在[保存并添加样品操作页面](#保存并添加样品操作页面)中提交审核，具体操作如下：
点击【提交审核】，弹出[提交审核确认对话框](#提交审核确认对话框)，如下图所示：

<html><span id='提交审核确认对话框'></span></html>

![提交审核确认对话框]()
点击【确定】弹出[提交审核成功提示对话框](#提交审核成功提示对话框)，如下图所示：

<html><span id='提交审核成功提示对话框'></span></html>
...
点击{实验室信息管理}-->{委托管理}-->{委托列表}→【委托审核】，弹出[委托审核页面](#orders/review/Page)，如下图所示：
...
操作路径：{实验室信息管理}→{委托管理}→{委托修改}→【操作】→【编辑】，弹出[委托修改页面](#orders/reeidt/page)，如下图所示：
